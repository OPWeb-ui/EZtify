...
import { Download, RefreshCcw, CheckCircle, FileText, Loader2, Info } from 'lucide-react';
...
export const WordToPdfPage: React.FC = () => {
  ...
  const onDrop = useCallback((acceptedFiles: File[], fileRejections: FileRejection[]) => {
    ...
    if (!f.name.toLowerCase().endsWith('.docx')) {
       addToast("Unsupported Format", "Only .docx files are supported.", "error");
       return;
    }
    ...
    try {
       ...
       setTimeout(async () => {
          try {
             ...
             console.error(e);
             addToast("Conversion Failed", "Could not convert this document. It might be complex or corrupted.", "error");
          } finally {
  ...
  return (
    <>
      <SEO 
        ...
        description="Convert Word (DOCX) documents to PDF in your browser. 100% private and client-side conversion."
        canonical="https://eztify.pages.dev/#/word-to-pdf"
      />
      <PageReadyTracker />
        ...
        {!file ? (
          ...
          >
            <section className="flex-1 flex flex-col items-center justify-center p-4 pt-8 pb-10 relative">
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] blur-[120px] rounded-full pointer-events-none bg-blue-500/10" />
                <motion.div 
                ...
                >
                  <motion.div variants={fadeInUp} className="w-full max-w-xl my-4 relative z-20">
                    <HeroPill>Convert DOCX documents to PDF instantly in your browser.</HeroPill>
                    <UploadArea onDrop={onDrop} mode="word-to-pdf" disabled={isGenerating} isProcessing={isProcessingFiles} />
                  </motion.div>
             ...
             <div className="w-full max-w-md bg-white dark:bg-charcoal-900 rounded-3xl shadow-xl border border-slate-100 dark:border-charcoal-800 p-8 text-center relative overflow-hidden">
                {!result ? (
                   <>
                      ...
                      <div className="bg-blue-50/50 dark:bg-blue-900/10 rounded-xl p-3 mb-6 text-xs text-blue-600 dark:text-blue-300 flex items-start gap-2 text-left">
                         <Info size={16} className="shrink-0 mt-0.5" />
                         <p>Best for standard documents. Complex layouts might not convert perfectly.</p>
                      </div>
                      ...
                      <div className="flex flex-col gap-3">
                         <motion.button
                           ...
                           className="w-full py-4 rounded-xl bg-blue-500 hover:bg-blue-600 text-white font-bold text-lg shadow-lg shadow-blue-500/20 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                         >
                            {isGenerating ? (
                              <>
                   ...
                   <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                      ...
                      <p className="text-sm text-charcoal-500 dark:text-slate-400 mb-6">Your document has been converted.</p>
