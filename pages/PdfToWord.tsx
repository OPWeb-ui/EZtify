...
import { 
  FileType2, RefreshCw, Lock, Cpu, Settings, Loader2, 
  ArrowRight, Layers, Trash2, CheckSquare, XSquare, ZoomIn, ZoomOut, Maximize, Minimize, FileText, Download, LayoutGrid, X
} from 'lucide-react';
...
export const PdfToWordPage: React.FC = () => {
  ...
  const handleConvert = async () => {
    ...
    try {
      // 1. Create a temporary PDF with only the kept pages
      setStatus('Preparing pages...');
      const trimmedPdfBlob = await savePdfWithModifications(file, pages, undefined, undefined, setStatus);
      const trimmedFile = new File([trimmedPdfBlob], "temp.pdf", { type: 'application/pdf' });

      // 2. Extract HTML from the trimmed PDF
      setStatus('Extracting text...');
      const html = await extractHtmlFromPdf(trimmedFile, setProgress, setStatus);

      // 3. Generate DOCX
      setStatus('Building Word document...');
      const docxBlob = await generateDocxFromHtml(html, setProgress, setStatus);
  ...
  if (!file) {
    return (
      <ToolLandingLayout
        title="PDF to Word"
        description="Convert PDF documents to editable Word (DOCX) files. Now supports page selection."
        icon={<FileType2 />}
        ...
        tip="You can delete unwanted pages before converting to Word to keep your document clean."
      />
    );
  }
      ...
      <div className="flex-1 flex flex-col min-w-0 relative z-10 bg-slate-100/50 dark:bg-black/20">
          ...
          <div className="h-14 border-b border-slate-200 dark:border-charcoal-800 bg-white dark:bg-charcoal-900 flex items-center justify-between px-4 shrink-0 shadow-sm z-20">
              <div className="flex items-center gap-4 text-xs font-mono text-charcoal-500 dark:text-charcoal-400">
                  <div className="flex items-center gap-2">
                      <span className="font-bold text-charcoal-700 dark:text-charcoal-200">FILE:</span>
      ...
      <div className="w-80 bg-white dark:bg-charcoal-900 border-l border-slate-200 dark:border-charcoal-800 flex flex-col shrink-0 z-20 shadow-[-4px_0_24px_rgba(0,0,0,0.02)]">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6">
              <div className="p-4 bg-slate-50 dark:bg-charcoal-800 rounded-xl border border-slate-200 dark:border-charcoal-700 space-y-3">
                  ...
                  <div className="flex justify-between items-center text-xs">
                      <span className="text-charcoal-500 dark:text-charcoal-400 font-mono">Pages to Convert</span>
              ...
              <div className="text-xs text-charcoal-500 dark:text-charcoal-400 leading-relaxed font-medium bg-blue-50 dark:bg-blue-900/10 p-3 rounded-lg border border-blue-100 dark:border-blue-900/20">
                 <span className="font-bold text-blue-600 dark:text-blue-400 block mb-1">Note:</span>
                 Only the pages visible in the list will be included in the final Word document. Deleted pages are excluded.
              </div>
          </div>
          ...
          <div className="p-4 border-t border-slate-200 dark:border-charcoal-800 bg-slate-50 dark:bg-charcoal-900 shrink-0 space-y-3">
              ...
              >
                  ...
                  <div className="relative flex items-center justify-center gap-2 z-10">
                      ...
                      <span>{isGenerating ? status || 'PROCESSING...' : 'EXPORT WORD DOC'}</span>
                  </div>
              </motion.button>
